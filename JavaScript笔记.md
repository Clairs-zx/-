...
>3. Array: 一种特别的`对象`==(特点：key为数值下标属性, 且内部数据是有序的)==

### Ⅲ-判断方法

#### ①*` typeof`*

>**`typeof`** 操作符返回一个==字符串==，表示**未经计算**的操作数的类型。
>
>* 可以判断: undefined/ 数值 / 字符串 / 布尔值 / function
>
>* 不能判断: null与object  object与array  `typeof null === 'object'`    `typeof [a,b,c] ==='object'`
>
...
>    //可以简化成 b3:()=>()=> 'hangzhou'  -->高阶函数相关知识
>    b3: function () {
>        return  () =>{  return   'hangzhou'}
>    }
>}
>/**使用instanceof判断对象是否是构造函数的实例*/
...
>
>/**调用对象与数组中某函数示例*/
>b1.b2[2]('调用console.log打印hangzhou')    //调用console.log打印hangzhou
>console.log(b1.b3()()) // hangzhou
> ```
...
>
>可以判断: undefined, null ，这两个类型只有一个值
>
...
>    b = null // 让b指向的对象成为垃圾对象(被垃圾回收器回收) {'atguigu': 12} 对象变为垃圾对象
>  ```
...
>   * 堆: 对象
>
>   * <img src="笔记中的图片/image-20210630192130518.png" alt="image-20210630192130518" style="zoom:50%;" />  
...
>
>ps:变量`obj.xx`-->`.`相当于拿着地址找到变量对应的内存,所以只有当变量中存的是地址,才可以用`.`
...
> ```js
> //2个引用变量指向同一个对象, 让其中一个引用变量指向另一个对象, 另一引用变量依然指向前一个对象
>//调用函数时参数传递是值传递，obj=a；即obj和a时两个对象保存的是相同的地  址，函数中执行的代码另obj指向另一块内存地址。
> //-->所以 a 仍是  {name: 'hong'}
...
>
>  ```js
>  //传进来的obj存储的是a中存的地址,所以obj==a(因为他们地址值一致,指向一致)
>  //2个引用变量指向同一个对象, 通过一个变量修改对象内部数据, 另一个变量看到的是修改之后的数据  -->所以被进行了修改
>    let a = {name: 'hong'}
>    const fn2=(obj) => obj.age= 15
>    fn2(a)   //值传递，传入函数的是a对象的堆地址 即obj指向a对象的地址
...
>    a = a + 1;
>    }
>    fn(a);  //函数调用的执行过程为，先读取a的值为3，并将3赋值给fn函数的形参a，函数中的形参a≠全局变量a。 所以输出的a为3，调用函数没有返回值，所以该步输出undefined
>    console.log(a)  //3
>  ```
...
>  var a = 3
>  var obj = {name:"Tom"}
>  obj = undefined ||null  //此时,obj没有被释放,但是之前声明的`{name:"Tom"}`由于没有人指向它,会在后面你某个时刻被垃圾回收器回收
> 
>function fn () { var b = {}}  //局部变量在函数调用时产生，在函数执行完自动释放，局部变量所指向的对象在间隔一段时间后被回收
...
>* ==方法==: 一种特别的属性(属性值是函数)， 本质是上一组特别的属性 `函数名： function(){}` 可缩写为`函数名(){}`
>  ```js
> var obj = {
>	name: 'Tom',  //属性名默认是字符串，无需加‘’
>	age: 12,
...
>* 函数声明
>* 表达式
>* 两种函数的区别在于变量提升，函数声明提升到最前面，表达式先定义变量值为undefined，执行带语句才赋值。
>
>  ```js
...
### Ⅱ-如何调用(执行)函数 (调用函数的几种方式？)

>1. test(): 直接调用
>
>2. obj.test(): 通过对象调用
>
>3. new test(): new调用  构造函数
>
>4. `test.call/apply(obj)`: ==临时让test成为obj的方法进行调用== 相当于`obj.test()` ,但`obj`上没有`test()`函数。此时`this`指向`obj`   **但注意函数不可使用箭头函数**  箭头函数的this指向window
>
>   * ![image-20210705185535337](笔记中的图片/image-20210705185535337.png)  
>
>5. 代码示例
>
>   ```js
...
>     this.xxx = 'Tom'
> }
> // obj.test2()  不能直接, 因为obj根本就没有test函数
> test2.call(obj)  // 可以让一个函数成为指定任意对象的方法进行调用
> console.log(obj.xxx)  //'Tom'
>       
>   ```
...
> // 定时器回调函数
> setTimeout(function () {
>   alert('到点了'+this)  //到点了window
> }, 2000)
>```

### Ⅳ-IIFE (自调用函数)

>1. 全称: `Immediately-Invoked Function Expression` 立即执行函数表达式 自调用函数 
>
>2. 作用:
>
>    * 隐藏实现   实现不是完全暴露外面的，外面的程序看不见
>    * 不会污染外部(一般指全局)命名空间
>    * 用它来编码js模块
>
>3. 代码示例
...
>* 任何函数本质上都是通过某个**对象**来调用的,如果没有直接指定就是window
>* 所有函数内部都有一个变量this
>* 它的值是`调用函数的当前对象`
...
>    }
>  
>Person("red"); //this是谁? ->window  此时Person只是个普通函数，等价于window.Person()
>  
>const p = new Person("yello"); //this是谁? -> p
>  
>p.getColor(); //this是谁? -> p  yellow
>  
>const obj = {};
...
## 1、 [原型与原型链](http://cavszhouyou.top/JavaScript%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3%E4%B9%8B%E5%8E%9F%E5%9E%8B%E4%B8%8E%E5%8E%9F%E5%9E%8B%E9%93%BE.html)
...
>  * ![image-20210714201049312](笔记中的图片/image-20210714201049312.png)
>2. 给原型对象添加属性(`一般都是方法`)
...
>       
> // 每个函数都有一个prototype属性, 它默认指向一个Object空对象(即称为: 原型对象)
> console.log(Date.prototype, typeof Date.prototype)  // {包含Data上方法的对象，供Data实例对象调用}  'object'
> function Fun () { }
...
>1. 每个函数function都有一个`prototype`，即`显式`原型(本质是**属性**，function也是的对象)
>
...
>4. 内存结构
>
>   ![image-20210714203043314](笔记中的图片/image-20210714203043314.png) 
>
>5. 总结:
...
>    * 如果最终没找到, 返回undefined
>    * ![image-20210714210912653](笔记中的图片/image-20210714210912653.png)
>  * 别名: 隐式原型链
>  * 作用: 查找对象的属性(方法) 
...
> 
>
>   ![image-20210714212928432](笔记中的图片/image-20210714212928432.png) 
>
>2. ```js
>     function Foo(){  }
> 
>  ![image-20210714212945164](笔记中的图片/image-20210714212945164.png) 
> 
...
>A.prototype.n = 1
>let b = new A()
>A.prototype = { n: 2, m: 3}  //A的原型指向了新的地址，此时b还指向原来的地址
>let c = new A()
...
>let f = new F()
>f.a() //a()
>f.b() //f.b is not a function -->找不到  ，因为f是个对象并不是个函数，所以他的原型链上没有Function.prototype
>F.a() //a()
>F.b() //b()
>
>console.log(f)
...
>结果图例
>
>![image-20210723173855550](笔记中的图片/image-20210723173855550.png)
...
>
> <img src="笔记中的图片/image-20210723181727823.png" alt="image-20210723181727823" style="zoom:80%;" /> 
 ...
 * 开始执行全局代码
3. 函数执行上下文
 * 在调用函数, 准备执行函数体之前, 创建对应的函数执行上下文**对象**(虚拟的, 存在于栈中)
 * 对局部数据进行预处理
   * 形参变量==>赋值(实参)==>添加为执行上下文的属性
   * `arguments`=>赋值(实参列表), 添加为执行上下文的属性，其为==伪数组== -->[不懂的同学看这里](https://developer.mozilla.org/zh-CN/docs/orphaned/Web/JavaScript/Reference/Functions/arguments)
...
![image-20210723183046182](笔记中的图片/image-20210723183046182.png)
...
var a 
console.log(a)  //f a() 不会被同名变量声明覆盖
console.log(a()) //10
a = 3    		// 变量赋值了
console.log(a)   //3
a = 6
console.log(a())   //此时a是6，不是个函数，所以报错a is not  a function
...
c(2) // 报错  c is not a function  此时c为1
```
...
> * 函数上下文环境==>对应的函数使用域
>
>![image-20210727141319410](笔记中的图片/image-20210727141319410.png)
...
>```
>
>![image-20210726192714660](笔记中的图片/image-20210726192714660.png)
...
一个函数和对其周围状态（**lexical environment，词法环境**）的==引用==捆绑在一起（或者说函数被引用包围），这样的组合就是**闭包**（**closure**）。也就是说，闭包让你可以在一个内层函数中访问到其外层函数的作用域。在 JavaScript 中，==每当创建一个函数，闭包就会在函数创建的同时被创建出来==。

-   如何产生闭包？

    -   当一个嵌套的内部函数（子函数）引用了嵌套的外部（父函数)的变量（函数）时，就产生了闭包
...
![image-20210727143351376](笔记中的图片/image-20210727143351376.png) 
...
![image-20210727143824641](笔记中的图片/image-20210727143824641.png)
...
4. 虽然好像没什么意义，但是很显然上述使用了[`闭包`]，而且该函数的返回值是一个函数。其实，这就是`高阶函数的定义：以函数为参数或者返回值是函数的函数。`
 ...
 ![image-20210415160945789](笔记中的图片/image-20210415160945789.png)

##### 2、柯里化

1. 图例:

 ![image-20210415161137977](笔记中的图片/image-20210415161137977.png)
...
![image-20210727191229838](笔记中的图片/image-20210727191229838.png)
...
不使用let const var等去声明,实际上是挂载到[`window`]上的,所以导致内存泄露![image-20210727193110329](笔记中的图片/image-20210727193110329.png)
...
alert(object.getNameFunc()());  //  the window 返回的函数没有调用getNameFunc函数中的局部变量
//函数体的this是window
...
方式三: 工厂模式
 * 套路: 通过工厂==函数==动态创建对象并返回 **本质是构建一个函数返回一个对象**
 * 适用场景: 需要创建多个对象
...
![image-20210728101320606](笔记中的图片/image-20210728101320606.png)
...
![image-20210728115630974](笔记中的图片/image-20210728115630974.png)  
...
2. 可以通过windows任务管理器查看进程
  - 可以看出每个程序的内存空间是相互独立的
  - <img src="笔记中的图片/image-20210728115541255.png" alt="image-20210728115541255" style="zoom:80%;" /> 
...
![image-20210728141032723](笔记中的图片/image-20210728141032723.png)
...
此处继续拿出栈图加深理解:<img src="笔记中的图片/执行栈与事件队列.gif" style="zoom:80%;" /> 
...
这里还有一张图来展示这个过程:<img src="笔记中的图片/image-20210729163242840.png" alt="image-20210729163242840" style="zoom:100%;" />

图中的stack表示我们所说的执行栈，web apis则是代表一些异步事件，而callback queue即事件队列,js引擎表示主线程，WebAPIs部分由浏览器处理。
...
![Promise系统学习_宏任务微任务原理图](笔记中的图片/Promise系统学习_宏任务微任务原理图.png) 
...
> ![image-20210729173545339](笔记中的图片/image-20210729173545339.png)
